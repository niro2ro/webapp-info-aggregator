...
| ファイル | 役割 |
|---|---|
| `DB起動.bat` | **PostgreSQL(Windowsサービス)を起動し、5432が受付可能になるまで待つ**共通ヘルパー。サービス名は `postgresql*` でワイルドカード照合（バージョン差異に非依存）。冪等（起動済みなら即戻る）。下記3つの先頭から呼ばれる |
| `収集バッチ.bat` | 収集バッチ（collection jar）を**1回だけ実行して終了**。初回は jar を自動ビルド。実行前に `DB起動.bat` でDBを担保。手動実行にも使える |
| `通知バッチ.bat` | 通知バッチ（notification jar）を**1回だけ実行**。未通知×お気に入りの新着を LINE に1通で送り冪等記録。実行前に `DB起動.bat` でDBを担保 |
| `自動実行.bat` | ログオン後に「**DB起動 → 収集 → 通知**」を順に実行するランチャー。DBが立たなければ中断し `logs\auto.log` に記録 |
...
- **DBの起動は各バッチが面倒を見る**: `DB起動.bat` が PostgreSQL(Windowsサービス)を起動し、ポート5432が受付可能になるまで最大約120秒待つ。以前のような固定待ち（5分）に頼らず、DB未起動での失敗を防ぐ。
  - 前提: サービスの**スタートアップの種類を「自動」**にしておくと確実（ログオン時に既に起動しており、非管理者でも待つだけで通る）。「手動」設定かつ非管理者だとサービス起動に失敗し得るため、その場合は「自動」に変更するか、事前にDBを起動しておく。
  - Docker Compose 運用の場合はサービスではないので、`docker compose up -d --wait`（`docker-compose.yml` に healthcheck 済み）を使うか、Docker Desktop をログイン時起動に設定する。
