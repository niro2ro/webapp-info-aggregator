...
| 本文抽出 | **jsoup** | LLM補完に渡す本文テキスト抽出用（HTMLスクレイピング取得＝専用パーサーは不採用） |
| RSS | **ROME (rome-tools)** | |
...
- **LLM はコストがユーザー数に比例しない構造**にする。呼び出しは収集時の構造化に限定。取得方式は **RSS のみ**（専用パーサー・LLM取得は廃止）。RSS で埋まらない発売日・不明カテゴリだけを取得後に LLM で補完し、LLM 使用を最小化する。
