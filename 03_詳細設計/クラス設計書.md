...
| 版 | 1.0（詳細設計・初版） |
| 作成日 | 2026-08-08 |
| 前工程 | [`アーキテクチャ設計書.md`](アーキテクチャ設計書.md)／`02_基本設計/`（承認済み） |
...
classDiagram
    ...
    class HtmlParser {
        <<interface>>
        +parse(String html, String url) RawItem
    }
    class LlmStructurer {
        <<interface>>
...
> `<<interface>>` はポート。`CollectionService` は**具象を知らず interface だけに依存**し、実装（ROME/jsoup/Claude/crawler-commons/JPA）は DI で注入される（差し替え・テスト容易）。
...
| `HtmlParser`（DD-CLS-13） | `JsoupHtmlParser` | jsoup | FR-02-02 |
| `HttpContentFetcher`（DD-CLS-14） | `RestClientHttpFetcher` | RestClient＋Resilience4j | FR-02-04/05 |
...
| 型 | 用途 | 備考 |
|---|---|---|
| `RawItem`（DD-CLS-25） | 取得直後の生データ（title/url/pubDate/description/category） | ROME/jsoup の出力を統一 |
