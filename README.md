...
- **冪等性**: 1日に複数回起動されうる前提のため、記事URLのハッシュに一意制約、通知履歴に複合キーを設け、**アプリ側のチェックではなく DB 制約で**重複登録・重複通知を防いでいます。
- **日時の扱い**: DB は `timestamptz` で UTC 保存、Java 側は `Instant` で保持し、表示直前に JST へ変換。ゾーンIDは定数へ集約し、コードに散在させていません。
- **収集先への配慮**: robots.txt の尊重、同一ホストへのリクエスト間隔制御、連絡先を含む User-Agent の送出。記事本文は保存・表示せず、**自作の要約と元記事URLのみ**を扱います。公式画像も自サーバーに保存しません。
