...
登録テーマにマッチした記事を発生日順に表示します。並び順（掲載日／発売日／収集日）・テーマ・情報源・カテゴリ・未読での絞り込み、タイトルと要約の全文検索（pg_trgm）に対応。
...
| **情報源マスタ（SC-06）**<br>情報源の登録・取得方式・有効フラグの管理<br>![情報源マスタ](docs/images/4.情報源マスタ.png) | **実行ログ（SC-08）**<br>収集ログ・通知ログ（送信結果と消費通数）<br>![実行ログ](docs/images/6.実行ログ.png) |
...
## 技術選定の理由（比較した候補と、選ばなかった理由）

各技術は候補を並べたうえで選んでいます。判断の経緯は [未決事項回答ログ](01_Requirements/未決事項回答ログ.md) に Q&A 形式で残してあります。

| 決めたこと | 採用 | 比較した候補 | 選んだ理由 / 見送った理由 |
|---|---|---|---|
| 言語・基盤 | **Java 21 + Spring Boot 3.3** | C# / .NET 8 + ASP.NET Core（当初案） | Java の実務経験を得るための学習を兼ねたポートフォリオであるため。要件（機能・画面・データ設計）を言語非依存に固めてあったので、**技術構成だけを差し替えて設計をそのまま流用できた** |
| Web UI | **Vaadin Flow** | ① Thymeleaf + htmx<br>② REST API + SPA（React 等） | サーバー側で UI 状態を持つ単一プロセス構成。**別途 JS フロントを学ばずに Java と Spring の理解へ集中できる**。利用者2名・ローカル運用には十分。②は学習範囲が二重になるため見送り |
| DB | **PostgreSQL** | SQLite | 将来の VPS 移行を前提にしたため。SQLite はファイル同梱型でサーバー運用に向かない |
| マイグレーション | **Flyway（SQL ベース）** | Hibernate の `ddl-auto` によるスキーマ自動生成 | **テーブル定義書の DDL をそのまま `V1__init.sql` に書ける**。スキーマの正を SQL 側に置き、ORM の自動生成に委ねない |
| 情報の取得方式 | **RSS のみ** | ① HTML スクレイピング（専用パーサー）<br>② LLM にページを読ませる | ①はサイト改修のたびに壊れ、保守コストが読めない。②はコストが処理量に比例して膨らむ。**RSS は構造が安定していて壊れにくい** |
| LLM | **Claude Haiku 4.5** | Claude Sonnet（当初の選定） | 当初は品質重視で Sonnet を選定したが、**実装して動かした結果、発売日・カテゴリの抽出精度は Haiku で十分**と判断。コスト削減のため変更した（[経緯](01_Requirements/未決事項回答ログ.md)） |
| ビルド構成 | **Maven マルチモジュール** | 単一モジュール | **層の依存方向を強制するため**。domain モジュールをフレームワーク非依存に保つ |
| バッチの実行形態 | **Spring Boot 別アプリ**（`CommandLineRunner`） | Web アプリに同居（スケジューラで定期実行） | バッチは「起動 → 処理 → 終了」する性質で、常駐する Web と混ぜると**障害の巻き込みとスケジューリングの複雑化**を招く |
| 通知手段 | **LINE Messaging API** | メール / Web Push | 日常的に見る導線であるため。なお LINE Notify は 2025-03 で終了しているため Messaging API を使用 |
