...
| `EventDateExtractor`（DD-CLS-22b） | **発売日(代表日)のルールベース抽出**。タイトル/要約の日本語日付表現（`2026年9月18日`/`9月18日`/`9月上旬`/`2026年春` 等）を正規表現で解釈し代表日＋精度＋原文を返す（無料・即時・LLM不使用）。取れない曖昧分はオンデマンドLLMへ | FR-02-03/06 |
