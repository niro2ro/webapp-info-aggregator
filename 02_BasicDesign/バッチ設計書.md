...
| 版 | 1.0（基本設計・初版） |
| 作成日 | 2026-08-08 |
| 前工程 | [`01_Requirements/要件定義書.md`](../01_Requirements/要件定義書.md) FR-02/FR-03・§8（承認済み v1.8） |
...
> 本書は**収集バッチ**と**通知バッチ**の処理フロー・入出力・冪等性の担保点・取得後のLLM補完・障害分離を確定する。取得方式は **RSS のみ**（専用パーサー・LLM取得は不採用）。両バッチは**別プロセス**で、**通知は LLM を一切呼ばない**（CLAUDE.md §5）。
...
| 本文テキスト抽出（LLM補完の入力用） | jsoup（※HTMLスクレイピング取得＝専用パーサーは不採用） |
...
flowchart TD
    ...
    D1 -->|なし| E1["抽出：タイトル・カテゴリ・種別・要約＋発売日をルールベース抽出（無料・LLM不使用）"]
    E1 --> INS["Articles へ INSERT：要約＋メタ＋元URL・本文/画像は保存しない（発売日はルールで取れた分のみ）"]
    INS --> M1["有効テーマと突合 → ArticleThemeMatches（分類不明はキーワードで拾う）"]
    M1 --> LOG["CrawlLogs 更新（収集ではLLMを呼ばない＝LlmUsageLogsは記録しない）"]
    SKIP --> LOG
    DUP --> LOG
    LOG --> LOOP
...
| BD-BATCH-C-02 | 取得方式 | **RSS のみ**（`fetch_type=1:Rss`）。専用パーサー・LLM取得は不採用。**収集では LLM を呼ばない**（発売日はルールベース抽出）。LLM補完はオンデマンド（下記 C-07 注記） | FR-02-01〜04 |
...
| BD-BATCH-C-07 | LLM予算ガード（**オンデマンド側**） | 収集では LLM を呼ばない。発売日補完の LLM は「発売日順」表示時や再解析（Web 側 `ArticleReanalyzeService`）で**オンデマンド**に走り、その**直前**に当月累計コスト（`LlmUsageLogs`・安全マージン込み）を判定。上限到達なら LLM を呼ばずルール結果のまま（収集・表示は継続） | FR-02-11・NFR-06 |
| BD-BATCH-C-08 | 抽出・マッピング | タイトル／カテゴリ／**発生日種別**／場所／要約 を抽出。**発生日（代表日）はルールベース抽出**（タイトル/要約の日付表現・無料・`EventDateExtractor`）で埋め、取れない曖昧な分はオンデマンドLLMへ委ねる。**要約は自作**（本文転載しない） | FR-02-06・§9 |
...
| BD-BATCH-C-12 | テーマ突合 | 収集記事を有効テーマのキーワードと突合し `ArticleThemeMatches` を永続化（複合PKで重複突合防止）。**カテゴリ絞り込みは分類が確定した記事のみに効かせ、分類不明（その他）はキーワード一致で拾う**（RSSにカテゴリが無い記事を取りこぼさない） | FR-02-10 |
| BD-BATCH-C-13 | ログ記録 | 情報源単位に `CrawlLogs`（件数・新規数・status・エラー）。**収集では LLM を呼ばない**ため `LlmUsageLogs` は記録しない（LLM のコスト記録はオンデマンド補完側で行う） | §8・FR-06-05/06 |
| BD-BATCH-C-14 | 障害分離 | 1情報源で例外→握って `CrawlLogs` に Failed/PartialError を残し次の情報源へ | NFR-10 |

> **取得・発売日・LLMの関係**: 取得方式は RSS のみ。**収集では LLM を呼ばない**（発売日はルールベース抽出で埋める＝無料・即時）。ルールで取れない曖昧な発売日・不明カテゴリだけを、「発売日順」表示時や再解析で**オンデマンドに LLM 補完**する（別工程・Web 側）。予算ガードで上限到達時は LLM を飛ばしてもルール結果のまま表示は継続。これで発売日順が基本 LLM 無し・即時、LLM 課金は最小になる。
