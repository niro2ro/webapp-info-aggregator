...
| BD-BATCH-C-02 | 取得方式 | **RSS のみ**（`fetch_type=1:Rss`）。専用パーサー・LLM取得は不採用。RSS で埋まらない項目のみ取得後に LLM 補完（C-06/07） | FR-02-01〜04 |
...
> **取得とLLM補完の関係**: 取得方式は RSS のみ。RSS で骨格（タイトル・日付・カテゴリ）が取れれば LLM 不要＝コスト最小。RSS で埋まらない発売日・不明カテゴリ等だけを取得後に LLM で補完する（専用パーサーは不採用）。予算ガード（C-07）で上限到達時は LLM を飛ばしても**収集自体は止めない**（RSS の結果で登録）。
