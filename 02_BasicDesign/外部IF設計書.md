# 外部インターフェース設計書 — 情報収集ツール（基本設計）

| 項目 | 内容 |
|---|---|
| 版 | 1.0（基本設計・初版） |
| 作成日 | 2026-08-08 |
| 前工程 | [`01_Requirements/要件定義書.md`](../01_Requirements/要件定義書.md) §7 外部I/F（承認済み v1.8） |
| 関連 | [`バッチ設計書.md`](バッチ設計書.md)／[`テーブル定義書.md`](テーブル定義書.md)／[`メッセージ・エラー一覧.md`](メッセージ・エラー一覧.md) |
| 設計ID体系 | `BD-IF-nn`。要件 `FR-02/03`・`TBL-*`・`NFR-06/11` を参照 |

> 本書は外部システムとの入出力を確定する。**インバウンド**＝収集対象サイト（**RSS のみ**）、**アウトバウンド**＝Claude API（収集時の構造化）・LINE Messaging API（通知）。**通知処理では Claude を呼ばない**（CLAUDE.md §5）。バージョン・料金・仕様は改定されうるため**実装着手時に各公式ドキュメントを確認**する（CLAUDE.md §0-4）。
...
## 1. インバウンド：収集対象（RSS のみ）

### 1.1 取得方式と、取得後の LLM 補完

取得方式は **RSS に一本化**（専用パーサー・LLM取得は不採用）。発売日は収集時に**ルールベース抽出**（`EventDateExtractor`・タイトル/要約の日付表現を正規表現で解釈・無料・即時）で埋め、取れない曖昧な分だけ「発売日順」表示時などに**オンデマンドで LLM 補完**する（LLM は取得手段ではなく取得後の構造化工程）。

| 段 | 方式 | ライブラリ | LLM | 使いどころ |
|---|---|---|---|---|
| 取得 | RSS/Atom | ROME（`SyndFeedInput`→`SyndFeed`/`SyndEntry`） | 不使用 | **唯一の取得方式**。タイトル・日付・カテゴリが構造化済み |
| 補完 | LLM 構造化 | anthropic-sdk-java（§2） | 使用 | RSS で埋まらない発売日・不明カテゴリ等のみ・予算内のみ・任意ON/OFF |
| （不採用） | 専用パーサー（jsoup） | jsoup | — | **HTMLスクレイピング取得は採用しない**（権利・保守コスト） |
...
| 使用モデル | 既定 **Claude Haiku 4.5**（`claude-haiku-4-5`・軽量/低コスト、発売日抽出は軽量タスクのため。設定 `LLM_MODEL` で変更可。正確なモデルID・単価・リクエスト形式は着手時に公式確認） |
...
| 項目 | 内容 |
|---|---|
| 方式 | **push**（友だち追加済みユーザーへ）。複数記事を **1吹き出し**に集約（Phase 4 初版は複数記事を1つのテキストメッセージにまとめて送信。見た目をリッチにする **Flex Message カルーセル化は後続の UX 改善**。無料枠の要は「吹き出し数=1」であり、テキスト集約でもこの制約は満たす） |
